...
def pending_recruitment():
    """
    Main script to generate a list of pending recruitment per month per HF. It
    is intended to be run daily, to check if there are pending Cohort recruitments
    based on the age range and the number of recruitments needed per HF, and the
    already recruited participants that month.
    """
    ...
    writer_summary = pd.ExcelWriter(SUMMARY_PATH)
    summary_sheet = None

    for project_key in tokens.REDCAP_PROJECTS_ICARIA:
        cohort_list_df = pd.read_excel(tokens.COHORT_RECRUITMENT_PATH,str(datetime.now().month))
        ...
        if project_key.split(".")[0] in cohort_list_df['HF'].unique():
            ...
            summary_sheet_part = excel_creation(project_key=project_key,redcap_project_df=df,excelwriter=writer)
    ...
    writer.close()
    file_to_drive(EXCEL_PATH)


def cohort_summary_script():
    """
    It creates the summary tool of actual cohorts per month per letter per HF recruited
    """
    ...
    for month in ['10']:
        ...
        month_expected,hf_per_month,nletter_list = cohort_summary_expected(month)
        for project_key in tokens.REDCAP_PROJECTS_ICARIA:
            if project_key.split(".")[0] in hf_per_month:
                ...
                if project_key == "HF13" and month=='06': #It was decided to include HF16 to get the rest of participants needed for HF13, because a lack of participants
                    stop = cohort_stopping_sistem(stop_df,nletter_list[project_key.split(".")[0]],project_key,date_="2023-" + month, min_age=min_age, max_age=max_age,additional=['HF13',['HF16.01', 'HF16.02', 'HF16.03']])
                else:
                    stop = cohort_stopping_sistem(stop_df,nletter_list[project_key.split(".")[0]],project_key,date_="2023-"+month,min_age=min_age2,max_age=max_age2)
                if project_key.split(".")[0] not in stop_dict:
                    stop_dict[project_key.split(".")[0]] = stop
                else:
                    pass
                group1_df = export_records_summary(project,project_key,['study_number','ch_his_date','ch_rdt_date'],"[ch_his_date]!=''",group1_df,month,min_age2,max_age2).fillna(0)
        print ("Groups Preparation . . . ")
        group =groups_preparation(group1_df,month_expected,stop_dict)
        print(group)
        print ("Writing the drive sheet . . .")
        file_to_drive_summary(month,group)


def GET_cohorts_from_this_month(redcap_project,projectkey, date_, min_age, max_age,additional=False):
    """
    This function is used to search for all cohort participants in one specific
    month for all REDCap subprojects on one specific HF that meets with age criteria
    ...
    if additional:
        ...
        for el in subprojects_to_check:
            print(el)
            project = redcap.Project(tokens.URL, tokens.REDCAP_PROJECTS_ICARIA[el])
            df = project.export_records(format='df', fields=params.ALERT_LOGIC_FIELDS)
            xres = df.reset_index()
        ...
        if "." in str(projectkey):
            list_subprojects = params.REDCAP_SUBPROJECTS[str(projectkey).split(".")[0]]
...
def cohort_stopping_sistem(redcap_project,nletter,projectkey,date_="-".join(str(date.today()).split("-")[:-1]),additional=False,max_age=False,min_age=False):
    """
    This function works searching if the number of cohorts participants per months
    has been reached. It takes into account several crtieria such as min age, max age,
    number of participants/HF, or study letter (since we don't know yet which letters
    are the Azi ones, we must collected a minimum number of participants per letter, or
    a determined number of participants per letter - some letter, following a formula
    described by the statistician L. Quinto in the protocol.
...
def get_record_ids_range_age(project_name,redcap_data,min_age,max_age,date_='2023-05-01'):
    """
    To find if list of participants are in the range age criteria. Used internally
    to get the list in other functions.
...
def get_record_ids_nc_cohort(project_key,redcap_data, max_age, min_age, nletter,projectkey,max_age2=False, min_age2=False, additional=False):
    ...
    """
    Needed to get all record IDs from those Non-contemporary cohort ICARIA participants
    This is one of the most important functions, since it runs all criteria to get the final
    candidate list per HF per month per letter.
...
def excel_creation(project_key,redcap_project_df, excelwriter,additional=False):
    """
    This script writes the excel sheet with candidates recruited and expected.
    """
    current_month = datetime.now().month
    ...
    if big_project_key.split(".")[0] in cohort_list_df['HF'].unique():
        ...
        if additional:
            ...
            all_to_FW, summary,stop = get_record_ids_nc_cohort(project_key,redcap_project_df, max_age=max_age, min_age=min_age,nletter=nletter,projectkey=project_key,additional=additional[:2])
        else:
            all_to_FW, summary, stop = get_record_ids_nc_cohort(project_key,redcap_project_df, max_age=max_age, min_age=min_age,nletter=nletter, projectkey=project_key, min_age2=min_age2,max_age2=max_age2,additional=False)
...
def cohort_summary_expected(month):
    """
    Get the expected number per HF per month per letter to be prompted in the Gsheet
    """
...
def groups_preparation(group,expected,finished_list):
    """
    It prepares the numbers recruited and expected, together, per HF per month per letter
    to be prompted in the sheet
    """
...
def additional_recruitments_from_another_hf(projects, mainproject,new_min_age,new_max_age):
    """
    This is an special case for HF16 and month june. A new range of age was established.
    """
    ...
    for project_key in projects:
        ...
        df = project.export_records(format='df', fields=params.ALERT_LOGIC_FIELDS)
        candidates_df = excel_creation(project_key=project_key, redcap_project_df=df, excelwriter=writer, additional=[mainproject,projects,new_min_age,new_max_age])
